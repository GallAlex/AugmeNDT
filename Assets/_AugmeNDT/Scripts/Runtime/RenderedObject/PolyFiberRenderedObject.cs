...
{
    private GameObject polyFiberInstance;
    ...
    {
        GameObject polyFiberContainerPrefab = (GameObject)Resources.Load("Prefabs/PolyModelContainer");
        polyFiberInstance = GameObject.Instantiate(polyFiberContainerPrefab, polyFiberContainerPrefab.transform.position, Quaternion.identity);
        polyFiberInstance.transform.SetParent(container.transform);
        ...
        polyFiberInstance.GetComponent<BoundsControl>().UpdateBounds();
    ...
    private void CreateCylinderRepresentation(PolyFiberData polyFiberDataset)
    {
        ...
        for (int fiber = 0; fiber < polyFiberDataset.NumberOfFibers; fiber++)
        {
            ...
            fiberObject.transform.SetParent(polyFiberInstance.transform);
